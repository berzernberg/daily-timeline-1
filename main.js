...
var OverlapDetector = class {
  static detectOverlaps(tasks) {
    ...
    for (const rangeTask of timeRangeTasks) {
      if (!rangeTask.endHour || rangeTask.endMinute === void 0) continue;
      ...
      for (const otherRange of timeRangeTasks) {
        ...
        if (!otherRange.endHour || otherRange.endMinute === void 0) continue;
...
var TimelineView = class extends import_obsidian.ItemView {
  ...
  async renderTimeRangeAsDot(segment, task, percentage, tagStyle) {
  ...
  async renderTimeRangeInSegment(segment, overlap, segmentWidth) {
    const task = overlap.task;
    if (!task.endHour || task.endMinute === void 0) return;
  ...
  async extractTasks(file) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (match) {
        ...
        const [, status, timeStart, timeEnd, content2] = match;
...
var EmbeddedTimeline = class {
  ...
  async renderTimeRangeInSegment(segment, overlap, segmentWidth) {
    ...
    const rangeWidthPixels = widthPercentage / 100 * segmentWidth;
    const tagStyle = this.getTagStyle(task.firstTag);
    if (rangeWidthPixels < DOT_THRESHOLD_WIDTH2) {
      await this.renderTimeRangeAsDot(segment, task, startPercentage, tagStyle);
      return;
    }
    const rangeContainer = segment.createDiv({ cls: "timeline-range-container" });
